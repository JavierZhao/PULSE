...
class Encoder(nn.Module):
    ...
    def __init__(self,sig_len=2500, window_len=100, in_chans=1,
                 embed_dim=1024, depth=8, num_heads=8, mlp_ratio=4., norm_layer=nn.LayerNorm, private_mask_ratio=0.5):
        super().__init__()
...
class CheapSensorMAE(nn.Module):
    ...
    def __init__(self, modality_name, sig_len=2500, window_len=100, in_chans=1,
        ...
        print(f"Initializing MAE for {modality_name}")
        ...
        self.encoder = Encoder(sig_len, window_len, in_chans, embed_dim, depth, num_heads, mlp_ratio, norm_layer, private_mask_ratio=private_mask_ratio)
    ...
    def forward_encoder(self, x, mask_ratio, ids_shuffle, ids_restore, ids_keep):
        ...
        private_embedding, shared_embedding, mask = self.encoder(x, mask_ratio, ids_shuffle, ids_restore, ids_keep)
        return private_embedding, shared_embedding, mask

    def forward_decoder(self, private_embedding, shared_embedding, ids_restore):
    ...
    def forward_with_visualization(self, sigs, mask_ratio=0.75):
        """
        A forward pass that also returns the data needed for visualization.
        """
        device = sigs.device
        num_patches = self.num_patches

        # Propose masking
        ids_shuffle, ids_restore, ids_keep = self.propose_masking(len(sigs), num_patches, mask_ratio, device)

        # Forward pass through encoder
        private, shared, mask = self.forward_encoder(sigs, mask_ratio, ids_shuffle, ids_restore, ids_keep)
    ...
    def predict_reconstruction(self, sigs_all,num_patches, mask_ratio=0.75):
        ...
        private, shared, mask = self.forward_encoder(sigs_all, mask_ratio, ids_shuffle, ids_restore, ids_keep)
        pred = self.forward_decoder(private, shared, ids_restore)  # [N, L, p*p*3]
        return pred
