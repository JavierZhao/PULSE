"""
Multi-modal contrastive (SimCLR-style) pretraining for physiological signals.

Pretraining strategy:
  1. For each modality, two augmented views of the same signal window are
     created (jitter, scale, shift, temporal mask).
  2. A Transformer encoder maps each view to a representation, which is then
     projected to a low-dimensional space.
  3. The NT-Xent loss maximises agreement between the two views of the *same*
     sample (positive pair) while pushing apart views of *different* samples
     (negative pairs — other items in the batch).
  4. Across modalities, an alignment loss (hinge) on the shared embedding
     subspace encourages cross-modal consistency (same as in the MAE
...
def train(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for m in models.values():
            m.train()
        train_contrastive_total, train_align_total = 0.0, 0.0
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # Per-modality contrastive loss
            contrastive_loss = 0.0
            all_shared_embs = {}
            for name, sig in signals.items():
                c_loss = models[name].contrastive_loss(sig)
                contrastive_loss += c_loss

                # Also collect shared embeddings for alignment
                priv, shared, _ = models[name].encoder(sig)
                all_shared_embs[name] = shared.mean(dim=1)  # pool over tokens

            contrastive_loss = contrastive_loss / len(signals)
            ...
            loss = contrastive_loss + args.alignment_loss_weight * align_loss
            loss.backward()
            ...
            train_contrastive_total += contrastive_loss.item()
            train_align_total += align_loss.item()
            pbar.set_postfix(loss=loss.item())

        avg_train_c = train_contrastive_total / len(train_dl)
        avg_train_a = train_align_total / len(train_dl)

        # --- Validate ---
        for m in models.values():
            m.eval()
        val_c_total, val_a_total = 0.0, 0.0
        with torch.no_grad():
            for batch in tqdm(val_dl, desc="Validating"):
                ...
                if len(shared_norm) >= 2:
                    a_loss = alignment_loss_fn(shared_norm)
                else:
                    a_loss = torch.tensor(0.0, device=device)
                val_c_total += c_loss.item()
                val_a_total += a_loss.item()

        avg_val_c = val_c_total / len(val_dl)
        avg_val_a = val_a_total / len(val_dl)
        avg_val_total = avg_val_c + args.alignment_loss_weight * avg_val_a

        logging.info(f"Epoch {epoch+1} | Train Contrastive: {avg_train_c:.4f} Align: {avg_train_a:.4f} | "
                     f"Val Contrastive: {avg_val_c:.4f} Align: {avg_val_a:.4f}")

        # --- TensorBoard scalars ---
        writer.add_scalar('Loss/train_contrastive', avg_train_c, epoch + 1)
        writer.add_scalar('Loss/train_align', avg_train_a, epoch + 1)
        writer.add_scalar('Loss/val_contrastive', avg_val_c, epoch + 1)
        writer.add_scalar('Loss/val_align', avg_val_a, epoch + 1)
        ...
        all_losses.append({
            'epoch': epoch + 1,
            'train_contrastive': avg_train_c, 'train_align': avg_train_a,
            'val_contrastive': avg_val_c, 'val_align': avg_val_a,
        })
...
def main():
    parser = argparse.ArgumentParser(description='Contrastive (SimCLR) Pretraining')
