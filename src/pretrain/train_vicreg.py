...
Pretraining strategy:
  VICReg (Variance-Invariance-Covariance Regularization) learns
  representations without negative pairs or momentum encoders by enforcing
  three complementary constraints on the batch embedding distribution:
    * Variance  — each dimension must maintain minimum std (prevents collapse)
    * Invariance — paired augmented views should have similar representations
    * Covariance — decorrelates embedding dimensions (prevents informational
      collapse where all dimensions encode the same feature)
...
def train(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for m in models.values():
            m.train()
        train_vic_total, train_align_total = 0.0, 0.0
        train_inv_total, train_var_total, train_cov_total = 0.0, 0.0, 0.0
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            vicreg_loss = 0.0
            batch_inv, batch_var, batch_cov = 0.0, 0.0, 0.0
            all_shared_embs = {}

            for name, sig in signals.items():
                vic_loss, components = models[name].vicreg_loss(sig)
                vicreg_loss += vic_loss
                batch_inv += components['invariance']
                batch_var += components['variance']
                batch_cov += components['covariance']

                # Shared embeddings for alignment
                priv, shared, _ = models[name].encoder(sig)
                all_shared_embs[name] = shared.mean(dim=1)

            vicreg_loss = vicreg_loss / len(signals)
            ...
            loss = vicreg_loss + args.alignment_loss_weight * align_loss
            loss.backward()
            ...
            train_vic_total += vicreg_loss.item()
            train_align_total += align_loss.item()
            train_inv_total += batch_inv / len(signals)
            train_var_total += batch_var / len(signals)
            train_cov_total += batch_cov / len(signals)
            pbar.set_postfix(loss=loss.item())

        n_batches = len(train_dl)
        avg_tr = {
            'vicreg': train_vic_total / n_batches,
            'align': train_align_total / n_batches,
            'inv': train_inv_total / n_batches,
            'var': train_var_total / n_batches,
            'cov': train_cov_total / n_batches,
        }

        # --- Validate ---
        for m in models.values():
            m.eval()
        val_vic_total, val_align_total = 0.0, 0.0
        with torch.no_grad():
            for batch in tqdm(val_dl, desc="Validating"):
                ...
                shared_embs = {}
                for name, sig in signals.items():
                    vl, _ = models[name].vicreg_loss(sig)
                    vic_loss += vl
                    _, sh, _ = models[name].encoder(sig)
                    shared_embs[name] = sh.mean(dim=1)
                vic_loss /= len(signals)
                ...
                val_vic_total += vic_loss.item()
                val_align_total += al.item()

        avg_val_vic = val_vic_total / len(val_dl)
        avg_val_align = val_align_total / len(val_dl)
        avg_val_total = avg_val_vic + args.alignment_loss_weight * avg_val_align

        logging.info(
            f"Epoch {epoch+1} | Train VICReg: {avg_tr['vicreg']:.4f} "
            f"(inv={avg_tr['inv']:.4f} var={avg_tr['var']:.4f} cov={avg_tr['cov']:.4f}) "
            f"Align: {avg_tr['align']:.4f} | "
        ...
        all_losses.append({
            ...
            'train_inv': avg_tr['inv'], 'train_var': avg_tr['var'], 'train_cov': avg_tr['cov'],
            'val_vicreg': avg_val_vic, 'val_align': avg_val_align,
        })
...
def main():
    parser = argparse.ArgumentParser(description='VICReg Pretraining')
    parser.add_argument('--run_name', type=str,
                        default=datetime.now().strftime("%Y%m%d_%H%M%S_vicreg"))
    parser.add_argument('--data_path', type=str,
                        default="/j-jepa-vol/PULSE/preprocessed_data/60s_0.25s_sid")
    parser.add_argument('--output_path', type=str,
                        default="/j-jepa-vol/PULSE/results/vicreg")
    ...
    # VICReg-specific
    parser.add_argument('--expand_dim', type=int, default=2048,
                        help='Expander MLP hidden/output dim')
    parser.add_argument('--inv_weight', type=float, default=25.0,
                        help='Invariance loss weight')
